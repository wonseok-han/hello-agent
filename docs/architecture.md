...
```
앱 실행
  ├─ 저장된 프로젝트 없음 → 위저드(첫 온보딩)
  └─ 저장된 프로젝트 있음 → 홈
                              ├─ 프로젝트 카드 → 편집기로 열기 / 첫 대화 이어가기
                              ├─ "새 프로젝트" → 위저드(진단·설치·로그인 완료분은 건너뜀)
                              └─ "문제 해결" → 닥터 재진단
```
...
**재방문 단축**: 이미 설치·로그인된 에이전트로 "새 프로젝트"를 시작하면 진단/설치/로그인을 건너뛰고 첫 프로젝트로 직행한다.

### 영속성 (홈베이스의 전제)

`tauri-plugin-store`(JS API, Rust 최소화 원칙)로 앱 설정 디렉터리에 JSON 저장:

- `projects[]`: `{ path, agent, name, createdAt, lastOpenedAt }` — 홈의 프로젝트 목록
- `lang`: 언어 선택 (기존 localStorage에서 이전)
- 프로젝트 생성(`create_first_project`) 시 기록, 홈에서 열 때 `lastOpenedAt` 갱신
